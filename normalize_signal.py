...
def normalize_signal_data(input_file, output_file, num_rows=10000):
    """
        ...
        for i, line in enumerate(f):
            ...
            if line.startswith('(') and line.endswith('j)'):
                ...
                if '+' in content:
                    ...
                    # 只有实部或虚部
                    if 'j' in content:
                        imag_part = float(content.replace('j', ''))
                        real_part = 0.0
                    else:
                        real_part = float(content)
                        imag_part = 0.0
    ...
    print(f"原始数据范围:")
    print(f"  实部: {np.min(complex_data.real):.2f} 到 {np.max(complex_data.real):.2f}")
...
if __name__ == "__main__":
    ...
    output_file_std = "signal_3.5_1.75_normalized_std.txt"
    
    print("=== 方法1: 按最大值归一化 ===")
    normalize_signal_data(input_file, output_file_max, num_rows=10000)
    
    print("\n=== 方法2: 按标准差归一化 ===")
    normalize_by_std(input_file, output_file_std, num_rows=10000)
    
    print("\n=== 转换完成 ===")
    print("您现在有两个归一化文件:")
    print(f"1. {output_file_max} - 按最大值归一化")
    print(f"2. {output_file_std} - 按标准差归一化")
